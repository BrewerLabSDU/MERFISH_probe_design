...
def add_readout_seqs_to_probes_of_transcript_balanced_random(probe_table:pd.core.frame.DataFrame, 
    ...
    Return:
        The updated table.
    '''
    # Check if the probe table is empty
    if probe_table.empty:
        print("Probe table is empty. Skipping.")
        probe_table['readout_names'] = pd.Series(dtype=str)
        probe_table['probe_barcode'] = pd.Series(dtype=str)
        probe_table['target_readout_sequence'] = pd.Series(dtype=str)
        return probe_table
